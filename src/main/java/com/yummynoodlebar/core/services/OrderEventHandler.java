...
public class OrderEventHandler implements OrderService {
	...
	public OrderCreatedEvent createOrder(CreateOrderEvent createOrderEvent) {
		...
		OrderStatusDetails orderStatusDetails = new OrderStatusDetails(event.getNewOrderKey(), UUID.randomUUID(), new Date(), "Order Created");
		SetOrderStatusEvent tmpEvent = new SetOrderStatusEvent(event.getNewOrderKey(), orderStatusDetails);
		OrderStatusEvent orderStatusEvent = ordersPersistenceService.setOrderStatus(tmpEvent);
